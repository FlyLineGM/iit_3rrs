...
*/
bool InertiaModel::dynScan(double zrange[3], double indices[2])
{
	double eigenval[3];
	...
	{
		z = zrange[1] + zrange[0] * i /ziter;	// Scanning across z range in steps of 5 mm
		...
		// Rotating Tilt axis 360 degrees in steps of 5 degrees
			...
			// Varying tilt angle in steps of 2 degrees
			for(int k=-phiiter; k<phiiter; k+=2)
			{
				phi[2] = k*M_PI/180;
				tanphiby2 = tan(phi[2]/2.);
				c1 = tanphiby2*phi[0];
				c2 = tanphiby2*phi[1];		
				task[0] = c1; task[1] = c2; task[2] = z;
